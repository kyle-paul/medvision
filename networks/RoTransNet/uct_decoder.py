...
class CCA(nn.Module):
    def __init__(self, F_d, F_e, size):
        super().__init__()
        self.size = size
    ...
    def forward(self, d, e):
        ...
        avg_pool_e = F.avg_pool2d(e, (self.size, self.size), stride=(self.size, self.size))
        channel_att_e = self.mlp_e(avg_pool_e)
        
        avg_pool_d = F.avg_pool2d(d, (self.size, self.size), stride=(self.size, self.size))
...
class UpBlockAtt(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, size):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2)
        self.coatt = CCA(F_d=in_channels//2, F_e=in_channels//2, size=size)
...
class UCTDecoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        df = config.df
        size = config.img_size // 4
        self.up3 = UpBlockAtt(df[2]*2, df[1], nb_Conv=2, size=size)
        self.up2 = UpBlockAtt(df[1]*2, df[0], nb_Conv=2, size=size*2)
        self.up1 = UpBlockAtt(df[1]  , df[0], nb_Conv=2, size=size*4)
